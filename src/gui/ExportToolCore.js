...
define(["jquery", "underscore-min", "jszip", "saveAs","./dialog/ErrorDialog"],
    function ($, _, JSZip, saveAs, ErrorDialog) {

        var self, mizarWidgetAPI, layers, availableLayers;
        ...
        function exportSelection(event) {
            ...
            _.each(availableLayers, function (layer) {
                if ($('#' + layer.layerId).is(':checked')) {
                    if (layer.type === mizarWidgetAPI.LAYER.OpenSearch || layer.type === mizarWidgetAPI.LAYER.GeoJSON) {
                        //if (layer.type === "DynamicOpenSearch") {
                        dataLayers.push(layer);
                    } else if (layer.type === mizarWidgetAPI.LAYER.Hips || layer.type === mizarWidgetAPI.LAYER.HipsCat) {
            ...
            _.each(dataLayers, function (dataLayer, index) {
                for (var i = 0; i < self.coordinates.length; i++) {
                    var tile = mizarWidgetAPI.getScene().tileManager.getVisibleTile(self.coordinates[i][0], self.coordinates[i][1]);
                ...
                var featureToStringify = {
                    geometry: {
                        ...
                        crs: {
                            type: "name",
                            properties: {
                                name: mizarWidgetAPI.getCrs().getGeoideName()
                            }
                        }
                    },
                    id: feature.id,
        ...
        return {
            ...
            init: function (mizar, options) {
                ...
                layers = mizarWidgetAPI.getLayers(mizarWidgetAPI.CONTEXT.Sky);
                self = this;
            },
